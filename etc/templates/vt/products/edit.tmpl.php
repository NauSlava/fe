<?php
    ...
    $__pageTitle = LocaleLoader::Translate( "vt.screens.product.editTitle");
	...
	$__breadcrumbs = array( 
		...
		, array( 'link' => Site::GetWebPath( "vt://products/edit/" . $objectId ) , 'title' => LocaleLoader::Translate( "vt.common.crumbEdit" ) ) 
	);
...
//echo $object->image->folderId.', ';
	...
	$j=1; $examples=''; $imagesarr=[];
	$query="SELECT * FROM objectImages WHERE objectId=".$objectId." AND objectClass='Product'";
	$query_id1=$db->query ($query);
	while ($row=$db->get_row($query_id1)) {
		if($row['smallImageId']!='') {
...
//			$imagesarr[0][0][$j]='<div id="vfsArea_smallImageId" class="fileinput"><a href="javascript:vfsSelector.Open( null, '.$row['smallImageId'].' );">Выбрать</a></div>';
			$imagesarr[0][0][$j]='<div id="vfsArea_smallImageId" class="fileinput">
				<a href="#smallImage" name="modal" class="ajax">Выбрать</a></div>';
		}
		...
		$examples.='<tr>
				...
				<td><a href="#" title="Удалить изображение" class="imagedel" data-object="'.$row['objectImageId'].'" data-ids="'.$imagesarr[0][2][$j].'" data-idb="'.$imagesarr[1][2][$j].'">
					<img src="/shared/images/vt/common/big-delete.gif" alt="Удалить изображение">
					</a></td>
			</tr>';
		$j++;
	};
...
//		return false;
	...
	function getImgs() {
//		console.log('id');
		var objectId=$('input[name="itemid"]').val();
...
<div class="main">
	<div class="inner">
		<form action="/shared/uploads.php" class="examplesimg hidden" method="post" target="hiddenframe" enctype="multipart/form-data" onsubmit="hideBtn();">
			<input type="hidden" id="itemid" name="itemid" value='{$objectId}'>
			<input type="hidden" id="cat" name="cat" value='Product'>
			<input type="hidden" id="path" name="path" value='{$path}'>
			<input type="hidden" id="action" name="action" value="addportfolioimage">	
			<label>Изображение для галереи:<br>
				<select id="folder" name="folder">{$folders}</select>
		...
		<form action="/shared/uploads.php" method="post" class="miniimg" target="hiddenframe" enctype="multipart/form-data" onsubmit="hideBtn();">
			...
			<input type="hidden" id="cat_s" name="cat_s" value='Product'>
		...
		<form action="/shared/uploads.php" method="post" class="bigimg" target="hiddenframe" enctype="multipart/form-data" onsubmit="hideBtn();">
			...
			<input type="hidden" id="cat_b" name="cat_b" value='Product'>

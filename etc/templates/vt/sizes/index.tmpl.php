<?php
    ...
    $__pageTitle = LocaleLoader::Translate( "vt.screens.size.list");

    $grid = array(
        "columns" => array(
           LocaleLoader::Translate( "vt.size.title" )
            , LocaleLoader::Translate( "vt.size.orderNumber" )
            , LocaleLoader::Translate( "vt.size.statusId" )
        )
        , "colspans"	=> array()
        , "sorts"		=> array(0 => "title", 1 => "orderNumber", 2 => "statusId")
        , "operations"	=> true
        , "allowAdd"	=> true
        , "canPages"	=> SizeFactory::CanPages()
        , "basepath"	=> Site::GetWebPath( "vt://sizes/" )
        , "addpath"		=> Site::GetWebPath( "vt://sizes/add" )
        , "title"		=> $__pageTitle
		, "description"	=> ''
        , "pageSize"	=> HtmlHelper::RenderToForm( $search["pageSize"] )
        , "deleteStr"	=> LocaleLoader::Translate( "vt.size.deleteString")
    );
